...
class Solution {
    ...
    void flow(vector<vector<int>>& heights, int r, int c, vector<vector<bool>>& reachSea) {
        if (reachSea[r][c]) return;
        reachSea[r][c] = true;
        int tmpHeight = heights[r][c];

        for (int i=0; i<4; i++) {
            int newR = r + direct[i][0];
            int newC = c + direct[i][1];
            if (newR<0 || newR>=heights.size() || newC<0 || newC>=heights[0].size()) continue;

            if (heights[newR][newC] >= tmpHeight) {
                // printf("(%d, %d)\n", newR, newC);
                flow(heights, newR, newC, reachSea);
            }
        }
    }
public:
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        for (int c=0; c<n; c++) {
            flow(heights, 0, c, reachPacific);
            flow(heights, m-1, c, reachAtlantic);
        }

        for (int r=0; r<m ;r++) {
            flow(heights, r, 0, reachPacific);
            flow(heights, r, n-1, reachAtlantic);
        }

        for (int r=0; r<m; r++) {
            for (int c=0; c<n; c++) {
                if (reachPacific[r][c] && reachAtlantic[r][c]) ans.push_back({r, c});
            }
        }
